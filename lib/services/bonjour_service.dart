...
class BonjourService {
  ...
  void addDiscoveryListener(Function(String deviceName, String ipAddress, int port) listener) {
    ...
    debugPrint('[BonjourService] Added discovery listener, total: ${_discoveryListeners.length}');
  }
  ...
  Future<void> start() async {
    ...
    debugPrint('[BonjourService] ========================================');
    debugPrint('[BonjourService] 🚀 Starting Bonjour service');
    debugPrint('[BonjourService] Platform: ${Platform.operatingSystem}');
    debugPrint('[BonjourService] ========================================');
  ...
  Future<void> _register() async {
    try {
      debugPrint('[BonjourService] Registering service: $alias');
      debugPrint('[BonjourService] Service type: $serviceType');
      debugPrint('[BonjourService] Port: $port');
  ...
  Future<void> _startDiscovery() async {
    try {
      debugPrint('[BonjourService] Starting discovery for: $serviceType');
      ...
      startDiscovery(serviceType).then((discovery) {
        ...
        debugPrint('[BonjourService] ✅ Discovery started successfully');
      }).catchError((e) {
  ...
  void _handleDiscovery() {
    ...
    for (final service in services) {
      ...
      debugPrint('[BonjourService] 📱 Discovered device: $deviceName');
      debugPrint('[BonjourService]    Hostname: $hostname');
      debugPrint('[BonjourService]    Port: $devicePort');
      debugPrint('[BonjourService]    TXT: ${service.txt}');
  ...
  Future<void> _resolveServiceMultipleWays(Service service, String deviceName, int devicePort, String? hostname) async {
    ...
    if (hostname != null && hostname.isNotEmpty) {
      try {
        debugPrint('[BonjourService] 🔍 Attempting DNS lookup for hostname: $hostname');
        final addresses = await InternetAddress.lookup(hostname).timeout(const Duration(seconds: 2));
        
        if (addresses.isNotEmpty) {
          debugPrint('[BonjourService] ✅ Got ${addresses.length} address(es) from DNS lookup');
    ...
    if (deviceName.isNotEmpty) {
      ...
      for (final variant in hostnameVariants) {
        try {
          ...
          if (addresses.isNotEmpty) {
            debugPrint('[BonjourService] ✅ Got addresses from variant: $variant');
            _notifyListeners(deviceName, devicePort, addresses);
            return;
          }
        } catch (e) {
    ...
    debugPrint('[BonjourService] ❌ All resolution strategies failed for $deviceName');
    debugPrint('[BonjourService] 💡 Device will still be discoverable via multicast');
  }
  ...
  void _notifyListeners(String deviceName, int devicePort, List<InternetAddress> addresses) {
    debugPrint('[BonjourService] 📡 Notifying listeners with ${addresses.length} address(es)');
    ...
    for (final address in addresses) {
      ...
      debugPrint('[BonjourService] ✅ Notifying: $deviceName @ $addressStr:$devicePort');
