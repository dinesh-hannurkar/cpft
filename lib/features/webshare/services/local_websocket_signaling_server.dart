...
/// Minimal local WebSocket signaling server (not Socket.IO).
/// Host device runs this; peers connect via ws://<host-ip>:<port>
/// Protocol (JSON messages):
/// {type: 'join', roomId: 'ROOM'} -> server responds with existing-peers
/// {type: 'offer', to: <peerId>, from: <myId>, roomId, sdp: {...}}
/// {type: 'answer', to: <peerId>, from: <myId>, roomId, sdp: {...}}
/// {type: 'ice-candidate', to: <peerId>, from: <myId>, roomId, candidate: {...}}
/// Server broadcasts: peer-joined, peer-left
class LocalWebSocketSignalingServer {
  // Singleton instance to ensure single server binding per app lifecycle
  static final LocalWebSocketSignalingServer _instance = LocalWebSocketSignalingServer._internal();
  factory LocalWebSocketSignalingServer() => _instance;
  LocalWebSocketSignalingServer._internal();
  HttpServer? _server;
  final Map<String, Map<String, WebSocket>> _rooms = {}; // roomId -> peerId->socket
  ...
  Future<void> start({int port = 8080}) async {
    if (kIsWeb) {
      AppLogger.e('Local WebSocket signaling server cannot run on web platform', tag: 'LocalWS');
      throw UnsupportedError('Local signaling server is not supported on web. Use remote signaling instead.');
    }
    ...
    for (int i = 0; i < maxAttempts; i++) {
      try {
        ...
        _server = server;
        _port = attemptedPort;
        AppLogger.i('Local WS signaling server listening on port $attemptedPort', tag: 'LocalWS');
        if (attemptedPort != port) {
          AppLogger.i('Note: Port $port was in use, using $attemptedPort instead', tag: 'LocalWS');
        }
        break;
      } on SocketException catch (e) {
        if (e.osError?.errorCode == 98 || e.osError?.errorCode == 48) {
          // Address already in use (Linux: 98, macOS: 48)
          AppLogger.w('Port $attemptedPort already in use, trying ${attemptedPort + 1}', tag: 'LocalWS');
          attemptedPort++;
        } else {
          rethrow;
        }
    ...
    if (_server == null) {
      throw Exception('Failed to start server after trying ports $port-${port + maxAttempts - 1}');
    }
        ...
        switch (msg['type']) {
          case 'join':
            ...
            if (isHost && _hostIp != null && _port != null) {
              ...
              AppLogger.i('Registered room $roomId with host $_hostIp:$_port', tag: 'LocalWS');
            }
            
            // existing peers
            final peers = room.keys.where((k) => k != peerId).map((k) => {'id': k}).toList();
            ...
            for (final entry in room.entries) {
              if (entry.key != peerId) {
                entry.value.add(jsonEncode({'type': 'peer-joined', 'id': peerId}));
              }
            }
            AppLogger.i('Peer $peerId joined room $roomId (existing=${peers.length})', tag: 'LocalWS');
            break;
          case 'offer':
          case 'answer':
          case 'ice-candidate':
  ...
  String _generatePeerId() => DateTime.now().microsecondsSinceEpoch.toRadixString(16);
}
