...
@objc class AppDelegate: FlutterAppDelegate {
  ...
  override func application(
    ...
    // Register Native QUIC Plugin (iOS 15+)
    if #available(iOS 15.0, *) {
        if let registrar = self.registrar(forPlugin: "QuicNativePlugin") {
            QuicNativePlugin.register(with: registrar)
            NSLog("[Fylooo] Registered QuicNativePlugin")
        }
    } else {
        NSLog("[Fylooo] Native QUIC not supported on this iOS version (<15.0)")
    }
